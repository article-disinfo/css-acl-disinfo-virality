# %% [markdown]
# # FakeNewsNet — Virality prediction (merged standalone)
# 
# Notebook autonome pour la tâche de viralité, sans exécuter d'autres notebooks.
# 
# Contenu:
# - Chargement des JSONL + embeddings pré-calculés (BERT/Mistral)
# - Cible de viralité: somme des `favorite_count` sur le chemin > médiane globale
...
def build_virality_targets(paths):
    totals = []
    for path in paths:
        total = 0.0
        for tweet in path:
            total += float(tweet.get('favorite_count', 0.0))
        totals.append(total)
    totals = np.asarray(totals, dtype=np.float32)
    thr = np.median(totals)
    y = (totals > thr).astype(int)
    return y, thr


def load_features_and_targets(mode='bert'):
    ...
    n = min(len(all_paths), len(X))
    X = X[:n]
    y, thr = build_virality_targets(all_paths[:n])
    return X, y, thr
...
for mode in ['bert', 'mistral']:
    print(f'\n=== Running virality ({mode}) ===')
    X, y, thr = load_features_and_targets(mode=mode)
    print('Shape:', tuple(X.shape), '| Positive rate:', round(y.mean(), 4), '| Median threshold:', round(float(thr), 4))
...
all_results.to_csv(OUT_DIR / 'fakenewsnet_virality_merged_cv_results.csv', index=False)
summary.to_csv(OUT_DIR / 'fakenewsnet_virality_merged_summary.csv')
print('Saved results to', OUT_DIR)


# %%
PUB_SUMMARY_FILENAME = 'fakenewsnet_virality_publication_summary.md'
